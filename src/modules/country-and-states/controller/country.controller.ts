...
export class CountryController {
  ...
  async getStateById(@Param('stateId') stateId: string) {
    return this.countryService.getStateById(stateId);
  }
  ...
  @Get('local-governments/:lgId')
  @ApiParam({ name: 'lgId' })
  async getLocalGovernmentById(@Param('lgId') lgId: string) {
    return this.countryService.getLocalGovernmentById(lgId);
  }
}
